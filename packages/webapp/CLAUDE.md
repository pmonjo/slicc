...
- Re-enabling after Stop: `ui/tray-join-url.ts:computeTrayMenuModel` returns `kind: 'leader-offer'` when both leader and follower are `inactive` (previously `'hidden'`, which removed the entire tray section from the avatar popover and stranded users who clicked Stop). The popover's `appendTrayMenu` renders an "Enable multi-browser sync" button that calls `leaveTray({ workerBaseUrl: <default> })` — the existing `kind: 'switched'` branch in `performTrayLeave` covers `inactive → leader` without a separate helper, and the four-transport selector keeps extension panel / standalone page / offscreen hook routing identical to the leave path.
