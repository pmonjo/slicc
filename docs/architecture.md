...
| `tray-follower-sync.ts`     | Follower-side `FollowerSyncManager` (TS — used by browser standalone follower and extension offscreen follower). Implements `AgentHandle`, receives agent events / snapshots / sprinkle messages from leader, advertises local browser targets, forwards CDP+FS for federated targets. `scoops.list` arrives but currently routes to the default no-op branch (no scoop switcher on the TS side yet — iOS consumes it for swipe navigation)                                                                                                                                                         |
...
- iOS `LeaderToFollowerMessage.init(from:)` has an `.unknown` fallback so the app silently drops messages it doesn't recognize. iOS `FollowerToLeaderMessage.init(from:)` throws on unknown types. The Swift file headers at `:145` and `:334` declare this asymmetry explicitly.
...
| Follower→Leader | `tab.open.error`                                            | TS only         | Declared in the union for symmetry with the leader→follower error reply. iOS's `CDPBridge.handleTabOpen` always sends `.tabOpened` (even when navigation fails); the TS follower sends this only when the local browser transport rejects target construction |
